  ...
  "Scripts/BookmarkManager/markdown.js"(exports2, module2) {
    ...
    function formatTags(tags) {
      ...
      return tags.map((tag) => {
        ...
        return name.trim();
      }).filter(Boolean).map((name) => `#${name.replace(/\s+/g, "-")}`).join(" ");
    }
    ...
    function buildBookmarkBlock(bookmark) {
      ...
      const type = bookmark.bookmark_type || config2.DEFAULT_BOOKMARK_TYPE;
      ...
      const createdAt = utils2.formatDate(
        bookmark.created_at,
        config2.DATE_LOCALE
      );
      const lines = [];
      lines.push(`<!-- BOOKMARK:${id} -->`);
      lines.push("");
      lines.push("> [!bookmark]");
      if (imageUrl) {
        lines.push(`> ![](${imageUrl})`);
      ...
      lines.push(">");
      if (domain) {
        lines.push(`> \u{1F310} ${domain}`);
        lines.push(">");
      }
      lines.push(`> \u{1F516} ${utils2.capitalize(type)}`);
      lines.push(">");
      if (url) {
        lines.push(`> \u{1F517} [${url}](${url})`);
        lines.push(">");
      }
      if (description) {
      ...
      if (tags) {
        lines.push(`> \u{1F3F7}\uFE0F ${tags}`);
        lines.push(">");
      }
      if (collections) {
        lines.push(`> \u{1F4C2} ${collections}`);
        lines.push(">");
      }
      if (createdAt) {
        lines.push(`> \u{1F4C5} ${createdAt}`);
      }
      lines.push("");
      lines.push("<!-- END BOOKMARK -->");
      return lines.join("\n");
    }
    module2.exports = {
      buildBookmarkBlock,
